...
public class codepageSevrlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        else if("searchstatus".equals(action))
        {
        	...
        	sql="select a.codeid,manageunit,cutnum,staff_id_1 from codepage a join codestatus b on a.codeid=b.codeid where status not like 'back'";
        ...
        else if("allback".equals(action))
        {
        	...
        	for(int i=0;i<json.length();i++)
        	{
        		 ...
        		 sql="select count(*) from codestatus where codeid="+each+" and status='again'";
         		double ff=cpd.findcount(sql);
         		if(ff>0)
         		{
         		sql="update codestatus set status='back' where codeid="+each+"";
    	...
    	else if("updateCodepage".equals(action))
    	{
        	...
        	if(f==1) {
        		flag=cpd.updateCodePage(cp);
        		sql="select count(*) from codestatus where codeid="+codeid+" and status='back'";
        		double ff=cpd.findcount(sql);
        		if(ff>0)
        		{
        		sql="update codestatus set status='again' where codeid="+codeid+"";
